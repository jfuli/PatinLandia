...
function generoTabla($conexion, $nomTabla): string {
    ...
    if ($nomTabla === 'Productos') {
        $tabla .= "<div class='text-center'>"
                . "<form action='gestor.php'  method='post'><input type = 'submit' value = 'Añadir producto' name = 'accion' class='shop-btn botonesPago my-4'>";
    }
    foreach ($filas as $fila) {
        ...
        if ($nomTabla === 'Productos') {
            $tabla .= "<tr>"
                    . "<td class='pago text-center no-border' colspan='2'>\n"
                    . "<input type = 'submit' value = 'Editar' name = 'accion' class='primary-btn botonesPago my-4'>"
                    . "</td></tr>";
        }
        $tabla .= "</form>";
        $tabla .= "</table><br>";
        if ($nomTabla === 'Pedidos') {
            ...
            $tabla .= "<div class='mx-auto text-center'>"
                    ...
                    . "<input type='submit' class='shop-btn' name='botonEstado' value='Guardar'>"
                    . "</form>"
                    . "</div>";
        }
